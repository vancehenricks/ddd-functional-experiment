...
export function validateNewUser(): any {
  ...
  return [
    ...
    getBody<UserRegistrationUnHashed>('username').isLength({ min: MIN_USERNAME_LENGTH,  max: MAX_USERNAME_LENGTH }).withMessage(`Username must be at within ${MIN_USERNAME_LENGTH} - ${MIN_USERNAME_LENGTH} characters long`),
